...
//    function setCardNumPerAddr(uint8 count_) external onlyOwner{
//        cardNumPerAddr = count_;
//    }
    ...
    function getTokenList(uint256 idx_) public view returns (bool,uint256){
        if(idx_ < tokenList[msg.sender].length){
            return (true,tokenList[msg.sender][idx_]);
        }
        return (false,0);
    }
    ...
    function MintByOperator(address user_,uint256 count_) external OnlyOperator{
        require(CardID<TotalCardId,"exceed total card count");
        require(tokenList[msg.sender].length<count_,"card count not correct");
        uint256 cnt=count_-tokenList[msg.sender].length;
