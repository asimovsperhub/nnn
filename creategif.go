...
func CreateGif(label string, cardColor string, savePath string) {
	...
	for i := 1; i < 251; i++ {
		...
		go func(i int, wg *WaitGroup) {
			...
			writeimage, err := writeLabel(g, label, 35, 35, color.RGBA{28, 223, 143, 255}, 30, "./PingFang Heavy.ttf")
			if err != nil {
				fmt.Println(err)
			}
			cp := getPalette(g)
...
func main() {
	// "color", "gold"
	for _, card := range []string{"green"} {
		cardColor := card
		beforeTime := time.Now()
		for i := 5329; i < 10000; i++ {
			label := "NO."
			scardId := fmt.Sprint(i)
			switch len(scardId) {
			case 1:
				label += "000" + scardId
			case 2:
			...
			savepath := fmt.Sprintf("./cards/%s/%s_%s.gif", cardColor, cardColor, scardId)
			CreateGif(label, cardColor, savepath)
		}
